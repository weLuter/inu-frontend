...
const Sidebar = (props) => {
  ...
  const { wrapperRef, sidebar, pageIndex, setPageIndex, handleNavigate, dashboardRef, portfolioRef, buybackRef, test } = props;
  ...
  return (
    <div ref={wrapperRef} className={`
        ...
        w-72 h-screen bg-gray-800
        transition ease-in-out duration-300
        ${sidebar ? "translate-x-full" : "transform-x-0"
      }`}>
          <div>
            <div className="px-8 h-24 w-full flex items-center bg-white">
            ...
            <ul className="px-8 mt-8">
              <li onClick={() => { setPageIndex(0); handleNavigate(dashboardRef)}} className={`flex w-full justify-between cursor-pointer items-center mb-6 ${pageIndex === 0 ? 'text-gray-300' : 'text-gray-600 hover:text-gray-500'}`}>
              ...
              <li onClick={() => { setPageIndex(1); handleNavigate(portfolioRef)}} className={`flex w-full justify-between cursor-pointer items-center mb-6 ${pageIndex === 1 ? 'text-gray-300' : 'text-gray-600 hover:text-gray-500'} ${test ? 'hidden' : ''}`}>
                <div className="flex items-center">
              ...
              <li onClick={() => { setPageIndex(2); handleNavigate(buybackRef)}} className={`flex w-full justify-between cursor-pointer items-center mb-6 ${pageIndex === 2 ? 'text-gray-300' : 'text-gray-600 hover:text-gray-500'} ${test ? 'hidden' : ''}`}>
                <div className="flex items-center">
